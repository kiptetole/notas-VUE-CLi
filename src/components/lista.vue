  ...
  <div>
    ...
    <ul class="list-group">
      <li v-for="(nota,index) in todo" v-bind:key="index" class="list-group-item">
        <detalleNota v-bind:msg="nota.msg" v-bind:prioridad="nota.prioridad" v-bind:fecha="nota.fecha" v-bind:hecho="nota.hecho" @hecho="hechono" @borrar="borrarNota"></detalleNota>
      </li>
    </ul>
  </div>
  ...
  export default  {
    ...
    methods: {
      ...
      borrarTareasCompletadas: function(){
        for (let i; i<this.todo.length; i++){
          if (this.todo[i].hecho){
            console.log(i);
          }
        }
